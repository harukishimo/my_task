...
- 保護画面は既存画面に加えて`/plan`（今日の段取り）と`/private`（プライベート一覧）を提供する。
- `TaskCategory = "default" | "private"`をドメイン型として定義する。
- APIの作成・更新入力で`category`を受け付け、未指定は`default`とする。
- Google Sheetsの既存A:O形式も読み込み可能とし、P列がない行は`default`へ正規化する。新しい書き込みはA:P形式で行う。
- 今日の段取りは、Tasks表の`plan_date`と`plan_order`でシングルタスク実行キューを管理する。優先順位は、依頼が必要、10分以内、3営業日以上の分解、残りは期日順の4段階とする。
- `parent_task_id`で親子タスクを関連付け、`estimated_workdays >= 3`のタスクは分解対象として子タスク作成を促す。
- 旧タイムスケジュールの`ScheduleItems`タブとAPIは既存データ互換のため保持するが、現行の段取り画面では表示・編集しない。
...
画面ルートは要件上の5画面に対応させる。ルートパスは実装時に短縮してもよいが、意味が分かる命名とする。
...
| M | comment | 文字列 | 任意の補足コメント、最大2000文字 |
| N | plan_date | YYYY-MM-DDまたは空 | 「今日の段取り」に登録した日付 |
| O | plan_order | 正の整数または空 | 当日の実行順 |
| P | category | default/private | タスクカテゴリ |
| Q:V | review関連 | 既存形式 | 工数・確認リマインド・完了予定時刻 |
| W | parent_task_id | UUIDまたは空 | 親タスクのID |
| X | requires_request | TRUE/FALSE | 他者への依頼が必要か |
| Y | is_quick_task | TRUE/FALSE | 10分以内に完了するか |
| Z | estimated_workdays | 0.25〜365 | 想定営業日数。3以上は分解対象 |
...
- 初回表示で全タスクを1回取得し、6画面の集計・絞り込み・段取り表示はクライアントまたはサーバーの共通ロジックで行う。
- 常時ポーリングは行わない。
...
type TaskCategory = "default" | "private";

type Task = {
  id: string;
  title: string;
  comment: string;       // 任意、最大2000文字
  ...
  isDeleted: boolean;
  planDate: string | null; // 今日の段取りに登録した日付
  planOrder: number | null; // 当日の実行順
...
| GET | `/api/schedule?date=YYYY-MM-DD` | 指定日の時間割取得 |
| POST | `/api/schedule` | タスク予定・自由予定の追加 |
| PATCH | `/api/schedule/:id` | 時間・メモの編集 |
| DELETE | `/api/schedule/:id` | 時間割から論理削除 |
