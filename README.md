...
「今日の段取り」では、優先度マトリクスから今日やるタスクを実行キューへドラッグし、上から1つずつ処理できます。
...
- `/due`: 今日まで
- `/matrix`: 優先度マトリクス
- `/plan`: 今日の段取り（マトリクスからシングルタスク実行キューへ配置）
...
「今日の段取り」では、記事の優先順位（依頼、10分以内、3営業日以上の分解、期日順）を表示し、優先度マトリクスから今日の実行キューへ移します。キューはドラッグ、上下ボタン、完了ボタンで上から1つずつ処理できます。3営業日以上のタスクは親タスクとして子タスクを追加でき、親子関係はTasks表で管理します。旧タイムスケジュールのAPIと`ScheduleItems`タブは既存データ互換のため残していますが、現在の画面では表示・編集しません。
