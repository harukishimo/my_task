...
describe("Google Sheets mapper", () => {
  it("round-trips a task and recalculates priority", () => {
    const task = inputToTask({ title: "  シート確認  ", comment: "  補足メモ  ", dueDate: "2026-07-27", isUrgent: true, isImportant: true, category: "private", parentTaskId: "parent-1", requiresRequest: true, isQuickTask: false, estimatedWorkdays: 3 }, new Date("2026-07-20T00:00:00.000Z"), "id-1");
    ...
    expect(mapped).toMatchObject({ parentTaskId: "parent-1", requiresRequest: true, isQuickTask: false, estimatedWorkdays: 3 });
